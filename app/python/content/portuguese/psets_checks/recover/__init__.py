...
def test_noimage():
    """lida com falta de imagem forense"""
    check50.run("./recover").exit(1)

@check50.check(compila)
def primeira_imagem():
    """recupera 000.jpg corretamente"""
    check50.run("./recover card.raw").exit(0, timeout=10)
    if check50.hash("000.jpg") != HASHES[0]:
        raise check50.Failure("imagem recuperada não corresponde")

@check50.check(compila)
def imagens_do_meio():
    """recupera imagens do meio corretamente"""
    check50.run("./recover card.raw").exit(0, timeout=10)
