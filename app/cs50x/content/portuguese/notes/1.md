...
- No arquivo `conditions.c`, transformamos os trechos de condição anteriores em um programa:
  ...
  - Para David compilar e executar este programa em seu sandbox, ele precisava executar o comando `cd src1` no terminal. Isso muda o diretório, ou pasta, para aquele em que ele salvou todos os arquivos de origem da aula. Em seguida, ele pode executar `make conditions` e `./conditions`. Com `pwd`, ele pode ver que está em uma pasta `src1` (dentro de outras pastas). E `cd` sozinho, sem argumentos, nos levará de volta à nossa pasta padrão no sandbox.
...
- Outros exemplos não abordados na aula estão disponíveis em "Código-fonte" para [Semana 1](../../weeks/1/).
